...
Thông thường, các slide được hiển thị trong trình duyệt web, nhưng bạn cũng có thể export chúng sang các file PDF, PPTX, PNG hoặc Markdown để share hoặc in ấn. Tính năng này có sẵn thông qua CLI command [`slidev export`](../builtin/cli#export).

Tuy nhiên, các tính năng tương tác trong slide của bạn có thể không khả dụng trong các tệp đã export. Bạn có thể build và host slide của mình như một ứng dụng web để giữ lại tính tương tác. Xem thêm [Building và Hosting](./hosting) để biết thêm thông tin.
...
Lưu ý rằng tất cả các slide trong file PPTX sẽ được export dưới dạng hình ảnh, vì vậy văn bản sẽ không thể chọn được. Ghi chú của người thuyết trình sẽ được truyền vào file PPTX theo từng slide.
...
Dưới đây là một số option phổ biến bạn có thể sử dụng với command `slidev export`. Để xem danh sách đầy đủ các options, tham khảo [tài liệu CLI](../builtin/cli#export).

### Export các bước click

Mặc định, Slidev export một trang cho mỗi slide với animation khi click bị vô hiệu hóa. Nếu bạn muốn export các slide với nhiều bước thành nhiều trang, hãy sử dụng option `--with-clicks`:
...
Bạn có thể chỉ định tên file output ra với option `--output`:
...
Trong trường hợp bạn muốn xuất bài thuyết trình của mình bằng phiên bản giao diện tối, hãy sử dụng option `--dark`:
...
Khi export sang PNG, bạn có thể loại bỏ background mặc định của trình duyệt bằng cách thêm tùy chọn `--omit-background`:

```bash
$ slidev export --omit-background
```

Background mặc định của trình duyệt là nền trắng hiển thị trên tất cả các cửa sổ trình duyệt và khác với các nền khác được áp dụng trong toàn bộ ứng dụng bằng cách sử dụng CSS. [Xem tài liệu Playwright](https://playwright.dev/docs/api/class-page#page-screenshot-option-omit-background). Bạn sẽ cần áp dụng thêm CSS cho ứng dụng để hiển thị độ trong suốt.

Dưới đây là một ví dụ cơ bản để bao phủ tất cả các backgrounds trong ứng dụng:
...
Nếu bạn thấy một số nội dung bị thiếu hoặc animations chưa hoàn thành trong PDF đã export, hãy thử thêm thời gian chờ trước khi export mỗi slide:
...
Nếu file PDF hoặc PNG thiếu Emoji, có khả năng môi trường của bạn thiếu font cần thiết (chẳng hạn như [Google's _Noto Emoji_](https://fonts.google.com/noto/specimen/Noto+Emoji)).
